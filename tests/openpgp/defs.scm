...
;; this is nicer
(define GPG' `(,(tool 'gpg) --no-permission-warning --always-trust)) ;; xxx
(define PINENTRY (string-append (getcwd) "/" (qualify "fake-pinentry")))

(define (tr:gpg input args)
  (tr:spawn input `(,@GPG' ,@args --output **out** **in**)))

(define (pipe:gpg args)
  (pipe:spawn `(,@GPG' ,@args --output - -)))
