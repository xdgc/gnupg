...
(let ((counter (make-counter)))
  (for-each-p'
   ...
   (lambda (armored-file) (counter))
   (list msg_ols_asc msg_cols_asc msg_sl_asc msg_oolss_asc
	 msg_cls_asc msg_clss_asc)))
...
(let ((counter (make-counter)))
  (for-each-p'
   ...
   (lambda (armored-file) (counter))
   (list msg_olsols_asc_multiple msg_clsclss_asc_multiple)))
  ...
  (for-each-p'
   ...
   (lambda (armored-file)
	...
	(pipe:spawn `(,@GPG' --verify)))
       (error "verification succeded but should not")))
   (lambda (armored-file) (counter))
   (list bad_ls_asc bad_fols_asc bad_olsf_asc bad_ools_asc)))
