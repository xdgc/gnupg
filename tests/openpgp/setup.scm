...
(for-each-p "Unpacking samples"
  (lambda (name)
    (letfd ((source (open (in-srcdir (string-append name "o.asc"))
			  (logior O_RDONLY O_BINARY)))
	    (sink (open name (logior O_WRONLY O_CREAT O_BINARY) #o600)))
...
(for-each-p' "Storing private keys"
  (lambda (name)
    (letfd ((source (open (in-srcdir (string-append "/privkeys/" name ".asc"))
			  (logior O_RDONLY O_BINARY)))
	    (sink (open (string-append "private-keys-v1.d/" name ".key")
			(logior O_WRONLY O_CREAT O_BINARY) #o600)))
	   (call-with-fds `(,@GPG' --dearmor) source sink STDERR_FILENO)))
  (lambda (name) (counter))
...
;; hmm, that doesn't quite work on windows yet...
;;
...
(lettmp (tmp)
	(dearmor (in-srcdir "pubring.pkr.asc") tmp)
	(call-check `(,@GPG' --yes --import ,tmp)))
