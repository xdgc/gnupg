...
#define IMC(A, B)                                                       \
  ...
  FFI_RETURN_POINTER (sc, IMC (infp,
                              IMC (outfp,
                                   IMC (errfp,
                                        IMC (pid, sc->NIL)))));
#undef IMC
...
do_es_fclose (scheme *sc, pointer args)
{
  FFI_PROLOG ();
  estream_t stream;
  FFI_ARG_OR_RETURN (sc, estream_t, stream, pointer, args);
  FFI_ARGS_DONE_OR_RETURN (sc, args);
  FFI_RETURN_ERR (sc, es_fclose (stream));
}
...
do_es_feof (scheme *sc, pointer args)
{
  ...
  FFI_RETURN_POINTER (sc, es_feof (stream) ? sc->T : sc->F);
}
