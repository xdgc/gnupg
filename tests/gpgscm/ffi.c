...
do_fdopen (scheme *sc, pointer args)
{
  ...
  SC_RETURN_POINTER (sc, sc->vptr->mk_port_from_file (sc, stream, kind));
}
...
do_rename (scheme *sc, pointer args)
{
  ...
  char *old;
  char *new;
  SC_ARG (sc, char *, old, string, args);
  SC_ARG (sc, char *, new, string, args);
  SC_ARGS_DONE (sc, args);
  if (rename (old, new) == -1)
    SC_RETURN_ERR (sc, gpg_error_from_syserror ());
  SC_RETURN (sc);
...
do_wait_processes (scheme *sc, pointer args)
{
  ...
  for (i = 0; i < count; i++)
    retcodes_list =
      cons (sc,
            sc->vptr->mk_integer (sc, (long) retcodes[count-1-i]),
            retcodes_list);

  SC_RETURN_POINTER (sc, retcodes_list);
}
...
#undef BUFFER_SIZE
  ...
  SC_ARG (sc, int, binary, bool, args);
  SC_ARGS_DONE (sc, args);
  ...
  if (binary && a_stat.st_size != b_stat.st_size)
    {
      if (verbose)
        fprintf (stderr, "Files %s and %s differ in size %u != %u\n",
                 a_name, b_name, a_stat.st_size, b_stat.st_size);
...
do_splice (scheme *sc, pointer args)
{
  ...
  if (args != sc->NIL)
    SC_ARG (sc, ssize_t, len, number, args);
  SC_ARGS_DONE (sc, args);
  while (len == -1 || len > 0)
    {
...
# define O_TEXT		0
  ...
  define_function (sc, fdopen);
  define_function (sc, close);
  define_function (sc, mkdtemp);
  define_function (sc, unlink);
  define_function (sc, unlink_recursively);
  define_function (sc, rename);
  ...
  define_function (sc, inbound_pipe);
  define_function (sc, outbound_pipe);
  ...
  /* Test helper functions.  */
  define_function (sc, file_equal);
  define_function (sc, splice);

  /* User interface.  */
  define_function (sc, flush_stdio);
  define_function (sc, enable_readline);
  define_function (sc, prompt);

  /* Configuration.  */
  ffi_scheme_eval (sc, "(define *verbose* %d)", verbose);
  ...
  define_ (sc, "*args*", args);

#if _WIN32
  define_ (sc, "*pathsep*", sc->vptr->mk_character (sc, ';'));
#else
  define_ (sc, "*pathsep*", sc->vptr->mk_character (sc, ':'));
#endif
