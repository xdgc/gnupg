...
const char *scmpath = "../tests/gpgscm";
size_t scmpath_len = 1;
...
/* Load the Scheme program from FILE_NAME.  If FILE_NAME is not an
   absolute path, and USE_PATH given, then it is qualified with the
   values in scmpath until the file is found.  */
static gpg_error_t
load (scheme *sc, const char *file_name, int use_path)
{
  gpg_error_t err;
  size_t n;
  char *directory;
  ...
  n = (use_path && file_name[0] != '/') ? scmpath_len : 1;
  for (directory = scmpath; n; directory += strlen (directory) + 1, n--)
    {
      if (use_path)
        {
